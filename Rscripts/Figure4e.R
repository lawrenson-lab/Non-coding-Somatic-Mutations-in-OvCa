...
aliquots.RNA <- read.table(file = 'data/RNA-seq/rnaseq_metadata.tsv', sep = '\t', header = T, quote = "", comment.char = "")
...
bm.genes <- getBM(attributes=c('ensembl_gene_id', 'hgnc_symbol', 'gene_biotype'),
                  filters = 'chromosome_name',
                  values = c(1:22,'X'),
                  mart = .ensembl)
bm.genes <- bm.genes[-grep(pattern = 'pseudogene', x = bm.genes$gene_biotype),]

x <- read.table(file = 'results/pcawgRNAseq_mutantNotMutant_global_new_aim2_v3.txt', header = T, sep = '\t', quote = '')
y <- read.table(file = 'results/aim2.EOC_HGSOC.minOverlap4.0bp.filt.txt', header = T, sep = '\t', quote = '')
x <- merge(bm.genes, x, by = 'ensembl_gene_id')
x <- merge(x, y)
x$log2FC <- log2(x$FC)
x$negLog10WilcoxPValue <- -log10(x$wilcox.pvalue)
xMax <- max(abs(x$log2FC[is.finite(x$log2FC)]))
pdf(file = 'figures/Figure4e.pdf', width = 180/25.4/4, height = 180/25.4/4, useDingbats = F)
p.cutoff <- 0.05
x$pass <- x$wilcox.pvalue <= 0.05
p <- ggplot(data = x[x$geneMedianGE>=10,], mapping = aes(x = log2FC, y = negLog10WilcoxPValue, label = hgnc_symbol, col = pass)) + 
  ...
  geom_hline(yintercept = -log10(p.cutoff), linetype = 2, col = 'black') + 
  ylab(expression(-log[10](p))) +
  geom_point(data = x[x$p.value <= 0.05 & x$wilcox.pvalue <= p.cutoff & x$nNonPromoters>0 & x$nMutated >= 3 & x$geneMedianGE>=10,], col = 'black', size = 1) +
  geom_text_repel(data = x[x$p.value <= 0.05 & x$wilcox.pvalue <= p.cutoff & x$nNonPromoters>0 & x$nMutated >= 3 & x$geneMedianGE>=10,],
                  show.legend = F, 
                  nudge_y      = 0.05,
                  direction    = "x",
                  angle        = 0,
  ...
  theme_classic(base_size = 6) + theme(axis.text = element_text(color = 'black'), plot.margin = margin(0,0,0,0,'in'))
ggMarginal(p = p, margins = 'x', type = "histogram", xparams = list(breaks=seq(floor(-xMax),xMax,1)))
dev.off()

write.table(x = x, file = 'results/Figure6e.txt', append = F, quote = F, sep = '\t', col.names = T, row.names = F)